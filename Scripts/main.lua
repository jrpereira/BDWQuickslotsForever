...
local function trigger_consumable_slot(slot)
  local ov=bind_overlay()
  if not valid(ov) then return false end
...
local InventoryAssignment
local InventoryHints
...
local function inventory_assignment_allowed()
  if Config.Enabled==0 or InputGate.dialogueActive then return false end
  local inventory=false
  for id in pairs(InputGate.blockingWidgets) do
    local n=id:lower()
    if n:find("pausemenu",1,true) then return false end
    if n:find("wbp_hub_",1,true) or n:find("/_unified/gamehub/",1,true)
        or n:find("wbp_inventory_quickslotbindoverlay_c",1,true) then inventory=true end
  end
  return inventory
end
local function install_input_gate_hooks()
  ...
  hook("/Script/CommonUI.CommonActivatableWidget:ActivateWidget",function() end,function(Context)
    if InventoryAssignment then InventoryAssignment:Invalidate() end
    ...
    if valid(o) then
      ...
      if InventoryHints and (class=="WBP_Inventory_QuickslotBindOverlay_C" or class=="WBP_Hub_NewInventory_C") then
        InventoryHints:Request()
      end
    end
  end)
  hook("/Script/CommonUI.CommonActivatableWidget:DeactivateWidget",function(Context)
    if InventoryAssignment then InventoryAssignment:Invalidate() end
    if InventoryHints then InventoryHints:Invalidate() end
    local o=unwrap(Context)
...
if Config.Enabled~=0 then
  InventoryAssignment=dofile(scripts.."/inventory_assignment.lua")({
    register=RegisterKeyBind,queue=ExecuteInGameThread,
    allowed=inventory_assignment_allowed,assign=trigger_consumable_slot,log=log,
  })
  -- Reject queued input across a map boundary without retaining any UObject.
  RegisterLoadMapPreHook(function()
    InventoryAssignment:Invalidate()
    if InventoryHints then InventoryHints:Invalidate() end
...
-- Optional legacy cleanup is now requested by mapping/menu lifecycle events.
-- Its persistent-mapping behavior is unchanged in this performance candidate.
...
local NativeKeys=dofile(scripts.."/native_key_icons.lua")({
  valid=valid,path=object_path,resolve=native_action,log=log,
  key=function(vk) local name=VK_TO_FKEY[vk]; return name and {KeyName=FName(name)} or nil end,
  has_brush=function(system,key) return system:HasBrushForKey(key,0,true) end,
  brush=function(system,key) return system:GetBrushForKey(key,0,true) end,
})
local function native_display_context()
  local system=get_live("RebelInputDisplaySystem")
  local input=get_live("CommonInputSubsystem")
  if not valid(system) or not valid(input) then return nil end
  return {system=system,keyboard=tonumber(safe(input,"CurrentInputType"))==0}
end
local function set_key_widget(w,vk,hold,context)
  return NativeKeys:Set(w,vk,hold,context)
end
if Config.Enabled~=0 then
  InventoryHints=dofile(scripts.."/inventory_hints.lua")({
    queue=ExecuteInGameThread,enabled=function() return Config.Enabled~=0 end,
    valid=valid,get=safe,overlay=bind_overlay,
    inventory=function()
      return get_live("WBP_Hub_NewInventory_C",function(o)
        local ok,active=pcall(function() return o:IsActivated() end)
        return ok and active==true
      end)
    end,
    set_key=set_key_widget,display=native_display_context,log=log,
  })
  -- One initial request also handles a Lua reload with assignment already open.
  InventoryHints:Request()
end
...
local function override_icons(ability,consumable,verbose)
  local display=native_display_context()
  ...
  for slot=1,4 do
    ...
    if valid(ab) and set_key_widget(safe(ab,direction),Config["Ability"..slot],Config["Ability"..slot.."Mode"],display) then n=n+1 end
    if valid(cb) and set_key_widget(safe(cb,direction),Config["Consumable"..slot],Config["Consumable"..slot.."Mode"],display) then n=n+1 end
  end
...
local function override_skill_wheel(verbose)
  ...
  RadialPaths:Visit(function(radial)
      for slot=1,4 do
        ...
        if set_key_widget(button,Config["Ability"..slot],Config["Ability"..slot.."Mode"],display) then n=n+1 end
      end
  end)
