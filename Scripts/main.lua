...
local function setup_worker(run,remember_success)
  return dofile(scripts..'/widget_setup.lua')({
    ...
    delay=ExecuteWithDelay,queue=ExecuteInGameThread,log=log,run=run,remember_success=remember_success,
  })
end
...
local function update_hud_once()
  ...
  if valid(h) then request_wheel_setup(h,'hud') end
