...
local function ensure_key_visual(w,vk,hold)
  ...
  if rec.vk~=vk then
    if pcall(function()
      local label=key_label(vk)
      rec.text.Font.Size=(vk==1 or vk==2 or vk==4 or vk==5 or vk==6) and 16 or 22
      rec.text:SetText(FText(label))
    end) then rec.vk=vk end
  end
  if rec.hold~=hold then
    if pcall(function() rec.line:SetRenderOpacity(hold==1 and 1.0 or 0.0) end) then rec.hold=hold end
  end
  if not rec.shown then
    if pcall(function() rec.box:SetRenderOpacity(1.0) end) then rec.shown=true end
  end
  -- Keep the RebelInputWidget only as the layout anchor; its native key artwork is hidden.
  pcall(function() if w:GetRenderOpacity()~=0.0 then w:SetRenderOpacity(0.0) end end)
  return true
end
...
local InputLifecycleBudget=250
local LastLifecycleError
...
if Config.Enabled~=0 then LoopAsync(500,function()
  ...
  local scheduled,err=pcall(function()
    ExecuteInGameThread(function()
      -- Share the surviving maintenance callback instead of chaining one-shot
      -- lifecycle callbacks. Alternate 1000/500 ms checks (750 ms nominal mean).
      InputLifecycleBudget=InputLifecycleBudget+500
      if Config.Enabled~=0 and InputLifecycleBudget>=750 then
        InputLifecycleBudget=InputLifecycleBudget-750
        local lifeOk,lifeError=pcall(enhanced_input_step)
        if not lifeOk then
          if LastLifecycleError~=tostring(lifeError) then log("Input lifecycle update failed: "..tostring(lifeError)); LastLifecycleError=tostring(lifeError) end
        else LastLifecycleError=nil end
      end
  ...
  if not scheduled then
    HudWorkPending=false
    if LastHudWorkError~=tostring(err) then log("Maintenance dispatch failed: "..tostring(err)); LastHudWorkError=tostring(err) end
  end
  return false
end) end
