...
do
  ...
  local ok,err=pcall(NotifyOnNewObject,
    ...
    function()
      if Config.Enabled~=0 and RecoveryWork then RecoveryWork:Request('inventory');RecoveryWork:Tick() end
    end)
...
RecoveryWork=dofile(scripts.."/event_work.lua")({
  ...
  enabled=function() return true end, -- stays available for DMM master-enable events
  run=function(name)
