...
local function request_recovery()
  ...
  if Config.Enabled~=0 then RecoveryWork:Request('input'); RecoveryWork:Request('inventory') end
...
local WidgetSetup=dofile(scripts..'/widget_setup.lua')({
  valid=valid,key=function(o,kind) return object_path(kind=='radial' and o or owning_hud(o) or o) end,enabled=function() return Config.Enabled~=0 end,
  delay=ExecuteWithDelay,queue=ExecuteInGameThread,log=log,
  run=function(context,kind)
    ...
    if kind=='switcher' and valid(switcher) and not same(switcher,context) then return end
    if not valid(switcher) or not wheel_children_ready(safe(h,'WBP_AA_Quickslots'),safe(h,'WBP_HUD_Quickslots')) then
      return 'children_missing'
    end
    if not Enhanced.ready then return end
    ...
    local ok,err=WheelLayout:Update(h,switcher,ability,consumable,Config.ShowBothWheels~=0,ax,ay,cx,cy)
    if not ok then log('Wheel layout setup failed: '..tostring(err));return end
    if override_icons(ability,consumable,false)~=8 then return end
    if Config.ShowBothWheels~=0 then hide_swap_prompt(h,ability) end
    return true
  end,
})
local function watch_widget(path,kind)
  local ok,err=pcall(NotifyOnNewObject,path,function(o)
    ...
    if not path or not path:find('/Engine/Transient',1,true) then return end
    if kind=='radial' then RadialPaths:Invalidate() end
    WidgetSetup:Request(o,kind)
  end)
...
local function update_hud_once()
  local h=get_live('WBP_GameHUD_C',function(o) return (object_path(o) or ''):find('/Engine/Transient',1,true)~=nil end)
  if valid(h) then WidgetSetup:Request(h,'hud') end
  RadialPaths:Visit(function(radial) WidgetSetup:Request(radial,'radial') end)
...
local function reconfigure_from_text(now)
  ...
  WidgetSetup:Invalidate()
  request_recovery()
  if Config.Enabled~=0 then RecoveryWork:Request('hud') end
