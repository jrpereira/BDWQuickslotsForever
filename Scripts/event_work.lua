...
return function(env)
  ...
  function api:Request(name)
    ...
    -- A duplicate event must not refill an in-flight retry budget.
    lanes[name]=lanes[name] or env.attempts or 6
    schedule(false)
  end
  ...
  schedule=function(retry)
    ...
    local function dispatch()
      ...
      for _,name in ipairs({'input','inventory','hud','cleanup'}) do
        ...
        local remaining=batch[name] or lanes[name]
        lanes[name]=nil -- consume same-batch dependency handoffs exactly once
        if remaining then
