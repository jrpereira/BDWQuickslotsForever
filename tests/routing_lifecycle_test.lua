...
local route='OW';local closes,setups,ensures=0,0,0;local closeOK,restoreOK=true,true
...
local env=setmetatable({Config={Enabled=1},Enhanced=state,valid=function(o)return o and o.valid end,
 ...
 fullname=function(o)return o.name end,gameplay_context_signature=function()return route,true end,
 ...
 ensure_context=function()ensures=ensures+1;return restoreOK end,PersistentInput={EnsureGameplay=function()ensures=ensures+1 end},log=function()end}, {__index=_G})
...
assert(closes==0 and setups==0 and ensures==6,'routing alone preserves input subscriptions')
restoreOK=false;assert(not step() and setups==0,'missing-context restoration failures remain retryable')
restoreOK=true;assert(step())
...
print('PASS routing: OW/combat retains subscriptions, missing contexts repair, owner/component changes rebind, failed close/restore retry and config rebind')
