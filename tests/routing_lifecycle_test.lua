...
local b=assert(source:find('-- Native actions are gated directly',a,true))
local function obj(n)return {name=n,valid=true}end
local sub,pi,input,pc,pawn=obj('sub'),obj('pi'),obj('input'),obj('pc'),obj('pawn')
local route='OW';local closes,setups,ensures=0,0,0;local closeOK,contextOK=true,true
...
local env=setmetatable({Config={Enabled=1},Enhanced=state,valid=function(o)return o and o.valid end,
 ...
 PersistentInput={EnsureGameplay=function()ensures=ensures+1;return contextOK end},log=function()end}, {__index=_G})
...
local beforeCloses,beforeSetups=closes,setups
contextOK=false;assert(step() and closes==beforeCloses+1 and setups==beforeSetups+1,
 'expired gameplay context closes the stale bridge scope and reconstructs it')
