...
local unrelated={valid=true,name='/transient/unrelated'};local ow={valid=true,name='IMC_OW.IMC_OW'}
local pi={valid=true,AppliedInputContexts={[unrelated]=10001,[ow]=-1}}
local route=signature(pi);assert(route==ow.name and scans==1 and names==2)
pi.AppliedInputContexts[unrelated]=nil
route=signature(pi);assert(route==ow.name and scans==2)
...
print('PASS: one input-context pass provides Quickslot gameplay routing; compact mouse labels')
