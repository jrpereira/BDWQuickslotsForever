...
local event={providerId='QuickslotsForever',revision=1,values={Enabled=1},changes={}}
callback(event);callback(event);assert(#jobs==1 and reads==0)
table.remove(jobs,1)();assert(reads==1 and applied==0)
committed=committed..'\nAbility2=50';event.revision=2;callback(event);table.remove(jobs,1)()
assert(reads==2 and applied==1)
committed='incomplete';event.revision=3;callback(event);table.remove(jobs,1)();assert(applied==1 and #logs==1)

-- Exercise the vendored public v1 subscriber without another Lua state's callbacks.
...
local api=assert(loadfile('Scripts/settings_api.lua','t',env))()
local count=0
api.subscribe('QuickslotsForever',function(received)
 assert(received.providerId=='QuickslotsForever' and received.revision==1)
 assert(received.values.Enabled==0 and received.changes.Enabled.old==1);count=count+1
end)
...
local env2=setmetatable({Config={Enabled=1},Enhanced=state,
 ...
 PersistentInput={Validate=function()return true end,CloseInventory=function()end},
 NativeActionGates={RestoreAll=function()return true end},update_native_action_gates=function()return true end,
 FormatSetup={Invalidate=function()end},RecoveryWork={Invalidate=function()end,Request=function()end},
...
print('PASS settings Apply: no polling, coalesced one-time reads, malformed config isolation, public subscriber revisions and in-place master disable/enable')
