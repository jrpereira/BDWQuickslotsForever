...
local config={InteractionMode=0}
...
for _,t in ipairs(targets) do
 assert(action('InputAction /Game/Input/'..t.action..'.'..t.action,true))
...
for i,t in ipairs(targets)do
 local candidate='InputAction /Game/Input/'..t.action..'.'..t.action
 assert(action(candidate,true)==(i>4),'Selective preserves native slot actions but suppresses wheel toggles')
 assert(action(candidate,false),'restoration must include every action captured in Independent')
 assert(mapping(t.action)==(i>4))
end
